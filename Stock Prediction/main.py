...
series = TimeSeries.from_dataframe(pd.DataFrame(scaled_data, columns=['Close']))
...
future_dates = pd.date_range(start=actual_dates[-1], periods=n_days, freq='D')
...
combined_dates = actual_dates.append(future_dates)
